...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <h5>Rating: ${product.rating.rate}</h2>
      <h5>Total Review: ${product.rating.count}</h2>
      ...
      <button id="details-btn" class="btn btn btn-outline-warning">Details</button></div>
      `;
...
const updateTaxAndCharge = () => {
  ...
  const priceConvertedValue = parseFloat(priceConverted).toFixed(2);
  if (priceConvertedValue > 200) {
    setInnerText("delivery-charge", 30);
    setInnerText("total-tax", priceConverted * 0.2);
  }
  if (priceConvertedValue > 400) {
    setInnerText("delivery-charge", 50);
    setInnerText("total-tax", priceConverted * 0.3);
  }
  if (priceConvertedValue > 500) {
